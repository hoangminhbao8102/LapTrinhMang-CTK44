...
namespace Server
{
    class Employee
    {
        ...
        public byte[] GetBytes()
        {
            ...
            place += 4;
            Buffer.BlockCopy(Encoding.UTF8.GetBytes(LastName), 0, data, place, LastName.Length);
            place += LastName.Length;
            Buffer.BlockCopy(BitConverter.GetBytes(FirstName.Length), 0, data, place, 4);
            place += 4;
            Buffer.BlockCopy(Encoding.UTF8.GetBytes(FirstName), 0, data, place, FirstName.Length);
        ...
        public Employee(byte[] data)
        {
            ...
            place += 4;
            LastName = Encoding.UTF8.GetString(data, place, LastNameSize);
            place += place + LastNameSize;
            FirstNameSize = BitConverter.ToInt32(data, place);
            place += 4;
            FirstName = Encoding.UTF8.GetString(data, place, FirstNameSize);
            place += FirstNameSize;
